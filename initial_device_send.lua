...
function SendItemsToAPI(CONSTANTS, data)
    ...
    for i,v in pairs(data) do
        local APIUrl = string.format('http://%s:%s/%s', CONSTANTS.API.IP, CONSTANTS.API.PORT, CONSTANTS.API.BASE .. '/' .. CONSTANTS.API.item)
...
function CreateUsage(CONSTANTS, data, item_id)
    ...
    local APIUrl = string.format('http://%s:%s/%s', CONSTANTS.API.IP, CONSTANTS.API.PORT, CONSTANTS.API.BASE .. '/' .. CONSTANTS.API.usage)
    local request_body, response_body = {}

    -- TODO: BLOCK_OBJECT_SPECIFIC
    request_body = string.format('item_id=%s&consumption_type=%s&consumption_amount=%s&address=%s&unit=%s&external_id=%s&min_value=%s&max_value=%s',
                                item_id,
                                CONSUMPTION_TYPE,
                                CONSUMPTION_AMOUNT,
                                data['address'],
